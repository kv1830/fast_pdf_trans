...
import magic_pdf.model.pdf_extract_kit as orgin_pdf_extract_kit
...
class CustomPEKModel:
    ...
    def __init__(self, ocr: bool = False, show_log: bool = False, **kwargs):
        ...
        current_file_path = os.path.abspath(orgin_pdf_extract_kit.__file__)
        # 获取当前文件所在的目录(model)
    ...
    def __call__(self, image):
        page_index = analyse_local_data.page_index
        ...
        for res_index, res in enumerate(ocr_res_list):
            ...
            future = None
            if self.apply_formula and res['category_id'] == 1 and adjusted_mfdetrec_res:
                res_txts = ocr_to_txts(new_image)

                if not res_txts:
                    LOGGER.error('res_txts is empty, index: %d, res_index: %d', page_index, res_index)
                    cv2.imwrite(str(output_dir / f'{page_index:04d}_{res_index:04d}.jpg'), new_image)

                future = executor.submit(is_contains_code, self.analyze_agent, res_txts)
                future_num += 1

            params_list.append((future, adjusted_mfdetrec_res, useful_list, new_image))

        future_index = 0
        for res_index, res in enumerate(ocr_res_list):
            future, adjusted_mfdetrec_res, useful_list, new_image = params_list[res_index]
            ...
            try:
                ...
                LOGGER.exception('is_contains_code occur exception, index: %d, res_index: %d', page_index, res_index)
        ...
        analyse_local_data.page_index += 1
        return layout_res

orgin_pdf_extract_kit.CustomPEKModel = CustomPEKModel